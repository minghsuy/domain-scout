...
def verify_api_key(
    ...
    expected_api_key = request.app.state.api_key
...
async def ready(request: Request) -> dict[str, Any]:
    ...
    cached: dict[str, Any] = state.ready_cache
    if cached and now - float(state.ready_cache_ts) < _READY_CACHE_TTL:
        return cached
    ...
    state.ready_cache = result
    state.ready_cache_ts = now
    return result


@router.post("/scan", response_model=ScoutResult, dependencies=[Depends(verify_api_key)])
async def scan(req: ScanRequest, request: Request) -> ScoutResult:
    ...
    try:
        scout = Scout(config=config, cache=state.cache)
