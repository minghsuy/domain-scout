...
async def ready(request: Request) -> dict[str, Any]:
    ...
    now = time.monotonic()
    cached: dict[str, Any] = request.app.state.ready_cache
    if cached and now - float(request.app.state.ready_cache_ts) < _READY_CACHE_TTL:
        return cached
    ...
    request.app.state.ready_cache = result
    request.app.state.ready_cache_ts = now
    return result

@router.post("/scan", response_model=ScoutResult)
async def scan(req: ScanRequest, request: Request) -> ScoutResult:
    ...
    try:
        scout = Scout(config=config, cache=request.app.state.cache)
...
async def cache_stats(request: Request) -> dict[str, Any]:
    ...
    stats = await loop.run_in_executor(None, request.app.state.cache.stats)
